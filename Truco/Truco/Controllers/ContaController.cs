...
namespace Truco.Controllers
{
    ...
    public class ContaController : Controller
    {
        ...
        public async Task<ActionResult> Entrar(EntrarViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    {
                        if (User.IsInRole("Conveniados"))
                        {
                            return RedirectToAction("Indice", "Autorizacoes", new { area = "ParaConveniados" });
                        }
        ...
        [AllowAnonymous]
        public ActionResult RegistrarConveniado()
        {
            return View();
        }
        ...
        //
        // POST: /Conta/RecuperarSenha
        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RecuperarSenha(RecuperarSenhaViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user == null || !(await UserManager.IsEmailConfirmedAsync(user.Id)))
                {
                    ...
                    return View("RecuperarSenhaConfirmacao");
                }

                var code = await UserManager.GeneratePasswordResetTokenAsync(user.Id);
                var callbackUrl = Url.Action("RedefinirSenha", "Conta", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                await UserManager.SendEmailAsync(user.Id, "[SYNS] - Redefinir senha", "Por favor, redefina sua senha clicando <a href=\"" + callbackUrl + "\">aqui</a>");
                //ViewBag.Link = callbackUrl;
                return View("RecuperarSenhaConfirmacao");
            }
        ...
        //
        // GET: /Conta/RecuperarSenhaConfirmacao
        [AllowAnonymous]
        public ActionResult RecuperarSenhaConfirmacao()
        {
            return View();
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RedefinirSenha(RedefinirSenhaViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            if (user == null)
            {
                ...
                return RedirectToAction("RedefinirSenhaConfirmacao", "Conta");
            }
        ...
        //
        // GET: /Conta/RedefinirSenhaConfirmacao
        [AllowAnonymous]
        ...
        //
        // POST: /Conta/ExternalLoginConfirmacao
        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
